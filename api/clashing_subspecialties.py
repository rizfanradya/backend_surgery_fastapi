...
def get_clashing_subspecialties(limit: int = 10, offset: int = 0, search: Optional[str] = None, clashing_subspecialties_id: Optional[int] = None, session: Session = Depends(get_db), token: str = Depends(TokenAuthorization)):
    ...
    if clashing_subspecialties_id:
        query = query.where(
            ClashingSubSpecialties.id == clashing_subspecialties_id)
    if search:
        query = query.filter(or_(*[
            cast(
                getattr(ClashingSubSpecialties, column), String).ilike(f"%{search}%")
            if getattr(ClashingSubSpecialties, column).type.python_type == str
            else cast(getattr(ClashingSubSpecialties, column), String).ilike(f"%{search}%")
