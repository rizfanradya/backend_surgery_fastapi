...
def get_sub_specialties_ot_types(limit: int = 10, offset: int = 0, search: Optional[str] = None, sub_specialties_ot_types_id: Optional[int] = None, session: Session = Depends(get_db), token: str = Depends(TokenAuthorization)):
    ...
    if search:
        query = query.filter(or_(*[
            cast(
                getattr(SubSpecialtiesOtTypes, column), String).ilike(f"%{search}%")
            if getattr(SubSpecialtiesOtTypes, column).type.python_type == str
            else cast(getattr(SubSpecialtiesOtTypes, column), String).ilike(f"%{search}%")
            for column in SubSpecialtiesOtTypes.__table__.columns.keys()
        ]))
